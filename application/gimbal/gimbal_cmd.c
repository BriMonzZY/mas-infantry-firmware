...
int32_t gimbal_angle_ctrl(uint8_t *buff, uint16_t len)
{
    ...
    if (len == sizeof(struct cmd_gimbal_angle))
    {
		...
		// 步兵云台结构pitch和yaw直接使用增量控制  加上鼠标辅助控制量
		gimbal_set_pitch_speed(p_gimbal, p_cmd->pitch / 10.0f - (float)p_rc_info->mouse.y * 0.03f);
		gimbal_set_yaw_speed(p_gimbal, p_cmd->yaw / 10.0f - (float)p_rc_info->mouse.x * 0.03f);
