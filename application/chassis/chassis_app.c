...
struct route_obj chassis_route_table[] =
{
    ...
    {MANIFOLD2_ADDRESS, "can1_0x500_to_0x600"},
...
void chassis_app_init(void)
{
    ...
    //soft_timer_register(chassis_info_push, p_chassis, 10);  // 向pc发布底盘信息
...
/**
  * @brief  chassis adjust key 底盘校准按键回调函数
  * @param
  * @retval void
  */
void chassis_user_key_handle(void)
{
    ...
    ef_del_env(BMI088_PARAM_KEY);  // 删除环境变量
    HAL_Delay(2000);
...
//	protocol_send(MANIFOLD2_ADDRESS, cmd_id + 0x4000, pdata, len);
    if (cmd_id == 0x0201)
    {
		protocol_send(MANIFOLD2_ADDRESS, cmd_id + 0x4000, pdata, len);
//		if(shift_flag) {
//			chassis_power_limit =  *(pdata + 24) + 100;
//		}
//		else {
//			chassis_power_limit = *(pdata + 24);
//		}
//        can2_std_transmit(0x779, (uint8_t *)chassis_power_limit, 8);
		
//		chassis_power_limit = *(pdata + 24);
//		if(shift_flag) {
//			chassis_power_limit += 150;
//			*(pdata + 24) = chassis_power_limit & 0xFF;
//			*(pdata + 25) = (chassis_power_limit>>8) & 0xFF;
//			can2_std_transmit(0x779, pdata + 24, 8);
//		}
//		else {
//			can2_std_transmit(0x779, pdata + 24, 8);
//		}
...
//		log_printf("chassis_power_limit : %d\n", chassis_power_limit);
    }
	
	if (cmd_id == 0x0202)
    {
		uint16_t chassis_volt = *(pdata + 3)<<8|*(pdata + 2);
		uint16_t chassis_current = *(pdata + 1)<<8|*(pdata + 0);
        chassis_power = (float)chassis_volt*(float)chassis_current/1000000;
		chassis_power_buffer = *(pdata + 8);
//		log_printf("%d %d %d %d\n", chassis_volt, chassis_current, (uint16_t)chassis_power, chassis_power_buffer);
	}
}
...
{
    *power = chassis_power;
    *buffer = chassis_power_buffer;
	*power_limit = chassis_power_limit;
}
